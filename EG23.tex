...
batchs of fuel), the output buffer of the speration facilities have been used as
input for the ``Plutonium'' data fo the ``Fuel Ressource in storage''. The reprocessed
uranium is sent after the sepration to a dedicateed storage used as input for the
``Reprocessed Uranium'' data of ``Fuel Ressource in storage''. 
...
\begin{figure}[h!]
    ...
    \caption{Fuel Ressource in Storage.}
...
%\begin{figure}[h!]
%    \centering
%    \subfigure[Fuel Ressource in Storage (zoomed)\label{fig:FC_Z}]       {\epsfig{figure=img/FuelInStorage_1_zoom,width=0.48\textwidth}}
...
reprocessing'' and in the ``Fuel Ressource in Storage''.
...
\begin{figure}[h!]
    ...
    \subfigure[Fuel Ressource in Storage\label{fig:SFC_2}]  {\epsfig{figure=img/FuelInStorage_2,width=0.48\textwidth}}
