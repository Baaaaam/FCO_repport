...
simulation have been summarized in Table \ref{tab:reactor}. As some liberty have
been taken with the number of batch, cycle lengh and the core inventory both the
specifications and the values inplemented in the Cyclus input appears. 
...
%% I have increase the size of the batch by 1 to avoid a bid bug I
%% experience... which we have corrected after... But I did not revert the natch modification 
      \hline
    \end{tabularx}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% NOTE: I am note sure it birng any interresing information, so I remove it and I
%% add the actual error made on the mass for each reactor.

% For the SFR 1 facility type, the cycle
...
%respectively: $0.73$ and $0.79~GWd/t$. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

This does not affect the composition
...
affected with a respective relative error : $0.01 \%$, $0.0 \%$ and $0.006 \%$. 
...
corresponds to the output quantity ``UNF waiting for reprocessing'' and
``Ressource in Storage''.
Notable are subtle differences in what these two storage facilities data
represent and their exact semantic meaning in the analysis spreadsheet.
...
R. Carlsen calculation and might differe from the exact EG01-23 transition
specifications. 
...
The Fuel fabrication of the additionnal Cycamore\cite{cycamore} module mix the two
...
\begin{figure}[h!]
    \centering
    \subfigure[Used Fuel in Cooling Storage]   {\epsfig{figure=img/usedFuelInCooling,width=0.48\textwidth}}
...
%% NOTE: I think this is referring to Fig 6.... if so, it needs more
%% explanation and more careful wording.  It is important to carefully use the
%% appropriate names for these results since the names are so similar.  This
%% is about ``fuel resource'' in storage and not ``spent fuel'' in storage.
%% Need to change figure captions as well for Fig 6 & 7.
