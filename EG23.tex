...
%%% PPHW NOTE: Would 136 assemblies per core & 25 assemblies per batch have
%%% been bad, too?

%%% PPHW NOTE: Why didn't we change the core inventory to result in a matching
%%% flow rate of fuel?
...
two deployed in 2030 and one in 2040. In Case 1, the SFR separation facilities have an
effectively infinite separation capacity, but the separation rate has been throttled
indirectly using the gradual fuel fabrication capacity
...
%% PPHW: but they never halt because the output buffers are infinitely large
...
batchs of fuel), the separation facilities' output buffers are the basis for
the ``Fuel Resource in Storage'' result.
...
%    \hline
    ...
    LWR UOX B                 &  3           \\
    SFR MOX A                 &  2           \\
    SFR MOX B                 &  1           \\ %% This is different from the text (that has now been removed)
    \hline
    \end{tabular}
...
%% PPHW: do we have per-stream throughput limits??
...
%% PPHW: Aren't these identical figures to Fig 1.  Do we need to repeat them?
...
been reprocessed. 
%% PPHW: IS this the reason?  or is it because we start swapping high breeding ratio
%%       reactors for low breeding ration reactors?
...
%% PPHW: I don't see this?  I see spent UOX fuel from new LWRs accumulating, but not SFR MOX?
With the
...
"catch-up".
%% PPHW: this doesn't make sense - the throughput of spent fuel from the low BR SFRs is higher 
%%      than the other SFRs
...
decommisioning of the LWRs and the consequent expansion of SFRs, 
less LWR used fuel is produced, allowing the reduction of
...
than in case 1 the plutonium need of the SFR fuel fabrication. Indeed the
accumulation of plutonium is limited to $\sim500~t$ (Figure \ref{fig:SFC_2}) versus $\sim1500~t$ in the
first case. Nevertheless, one can still observe a large plutonium accumulation
spike 2100 and 2145 that could be avoided with a better reprocessing
capacity deployement.

%% PPHW: do you think it would be possible to place 8 figs on the same page, in 2 columns:
%%   current figs 4-6 in the first column and corresponding figs for Case 2 in the 2nd colum
%%   It would be much easier to compare the results.
...
SFR deployments replacing aging LWRs. Some of SFR fuel also accumulates
...
etc.) were ambiguous.  Much of the discrepancy between Cyclus' results and those of 
other simulators can likely be attributed to this factor.

Although Cyclus is not currently able to simulate on-demand behavior directly, it is
...
in Case 2.  There are some small differences in the pattern of fuel
